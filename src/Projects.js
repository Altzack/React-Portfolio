const projects = [
  {
    title: 'GlassPass',
    desc:
      'Buy, Sell, & Auction Glass Art on GlassPass (now with Crypto!) | The Most Trusted Glass Marketplace. My current job',
    img: 'images/glasspass1.png',
    appLink: 'https://glasspass.co/',
  },
  {
    title: 'CryptoTrack',
    desc:
      'A crypto currency tracker/dashboard for the top 100 CMC ranked crypto currencies.',
    img: 'images/crypto.png',
    langs: 'React, CSS',
    repoLink: 'https://github.com/Altzack/CryptoCurrencyTracker',
    appLink: 'https://cryptotrack-eight.vercel.app/',
  },
  // {
  //   title: 'Covid Tracker',
  //   desc: 'A covid dashboard with daily updated data.',
  //   img: 'images/covid.png',
  //   langs: 'React, CSS',
  //   repoLink: 'https://github.com/Altzack/Covid-Tracker',
  //   appLink: 'https://covidtrack1.netlify.app/',
  // },

  {
    title: 'Boozy',
    desc:
      'A platform to Browse, create and share custom mixed drink recipes with your friends, family, etc. This was built as my first full-stack capstone for Bloc.',
    img: 'images/boozy.png',
    langs: 'PostgreSQL, Express, React, Node',
    repoLink: 'https://github.com/Altzack/Boozy',
    appLink: 'https://boozy.vercel.app/',
  },
  {
    title: 'Stock-Dash',
    desc:
      'A dashboard that allows you to get the latest news and pricing for any NASDAQ traded symbol.',
    img: 'images/stockdash.png',
    langs: 'PostgreSQL, Express, React, Node',
    repoLink: 'https://github.com/Altzack/Stock-Dash',
    appLink: 'https://stock-dash.vercel.app/',
  },
];
export default projects;
